...
export async function multiFlowAllInOneProcessor(
  ...
  switch (ctx.appConfig.processingMode.MULTI_FLOW_PROCESSING_PHASE) {
    case MultiFlowProcessingPhase.INITIAL:
      await handleInitialPhase(ctx, parsedData);
      break;
    case MultiFlowProcessingPhase.HIST_DATA_AGGREGATION:
      await handleHistoricalDataAggregationPhase(ctx, parsedData);
      break;
    case MultiFlowProcessingPhase.SPOT_PRICES_CALCULATION:
      await handleSpotPricesCalculationPhase(ctx, parsedData);
      break;
    default:
      throw new Error(`MULTI_FLOW_PROCESSING_PHASE is misconfigured`);
  }
}

async function handleInitialPhase(
  ctx: SqdProcessorContext<Store>,
  parsedData: BatchBlocksParsedDataManager
) {
...
async function handleHistoricalDataAggregationPhase(
  ctx: SqdProcessorContext<Store>,
  parsedData: BatchBlocksParsedDataManager
) {
  await prefetchPersistentDataForMultiFlowProcHistDataAggregationPhase(ctx);
...
async function handleSpotPricesCalculationPhase(
  ctx: SqdProcessorContext<Store>,
  parsedData: BatchBlocksParsedDataManager
) {
  await prefetchPersistentDataForMultiFlowProcPricesCalcPhase(ctx);
