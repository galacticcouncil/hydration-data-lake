export const aggregateStablepoolVolumesByBlocksRange = `
    WITH stableswap_volume_start_block AS (SELECT id,
         ...
         stableswap_volume_end_block AS (SELECT id,
                                         ...
                                         WHERE pool_id = ANY ($1)
                                           AND para_block_height <= $3
                                           AND para_block_height >= $2),
         asset_volumes AS (SELECT sahv.volumes_collection_id,
                                  json_agg(
                                          json_build_object(
                                                  'id', sahv.id,
                           ...
                           WHERE para_block_height <= $3 AND para_block_height >= $2
    ...
    FROM (SELECT start_entity.pool_id AS pool_id,
                 ...
                 start_entity,
                 end_entity
          FROM (SELECT * FROM stableswap_volume_start_block WHERE rank = 1) AS start_entity
                   LEFT JOIN
             ...
             LEFT JOIN asset_volumes av_start_entity
                       ON grouped_data.start_entity_id = av_start_entity.volumes_collection_id
