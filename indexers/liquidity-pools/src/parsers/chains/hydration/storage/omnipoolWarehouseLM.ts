...
> {
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (
        storage.omnipoolWarehouseLm.globalFarm.v138.is(block) ||
        block.specVersion >= 138
      ) {
        try {
          const resp =
            await storage.omnipoolWarehouseLm.globalFarm.v138.getMany(
              block,
...
async function getAllDepositsData({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.omnipoolWarehouseLm.deposit.v138.is(block)) {
        try {
          const pairsPaged: OmnipoolYieldFarmDepositDataWithId[] = [];

          for await (const page of storage.omnipoolWarehouseLm.deposit.v138.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
...
> {
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (
        storage.omnipoolWarehouseLm.deposit.v138.is(block) ||
        block.specVersion >= 138
      ) {
        try {
          const resp = await storage.omnipoolWarehouseLm.deposit.v138.getMany(
            block,
