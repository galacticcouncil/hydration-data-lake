...
function parseDepositParams(event: SqdEvent): BalancesDepositEventParams {
  if (events.balances.deposit.v324.is(event)) {
    const { who, amount } = events.balances.deposit.v324.decode(event);
    return { who, amount };
  }
...
function parseWithdrawParams(event: SqdEvent): BalancesWithdrawEventParams {
  if (events.balances.withdraw.v324.is(event)) {
    const { who, amount } = events.balances.withdraw.v324.decode(event);
    return { who, amount };
  }
...
function parseReservedParams(event: SqdEvent): BalancesReservedEventParams {
  if (events.balances.reserved.v324.is(event)) {
    const { who, amount } = events.balances.reserved.v324.decode(event);
    return { who, amount };
  }
...
function parseUnreservedParams(event: SqdEvent): BalancesUnreservedEventParams {
  if (events.balances.unreserved.v324.is(event)) {
    const { who, amount } = events.balances.unreserved.v324.decode(event);
    return { who, amount };
  }
