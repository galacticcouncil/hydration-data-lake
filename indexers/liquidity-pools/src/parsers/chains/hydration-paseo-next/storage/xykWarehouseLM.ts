...
async function getXykpoolLMDeposits({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (
        storage.xykWarehouseLm.deposit.v324.is(block) ||
        block.specVersion >= 324
      ) {
        try {
          const resp = await storage.xykWarehouseLm.deposit.v324.getMany(
            block,
...
async function getAllDepositsData({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.xykWarehouseLm.deposit.v324.is(block)) {
        try {
          const pairsPaged: XykpoolLMDepositDataWithId[] = [];

          for await (const page of storage.xykWarehouseLm.deposit.v324.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
