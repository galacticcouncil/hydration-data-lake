...
async function getBond({
  ...
  if (storage.bonds.bonds.v324.is(block)) {
    const resp = await storage.bonds.bonds.v324.get(block, bondId);
    if (!resp) return null;
    return {
      bondId,
...
async function getBondsAll({
  ...
  if (storage.bonds.bonds.v324.is(block) || block.specVersion >= 324) {
    return tryExecOrReturnFallback(async () => {
      const pairsPaged = [];

      try {
        for await (const page of storage.bonds.bonds.v324.getPairsPaged(
          500,
          block
        ))
          pairsPaged.push(
