...
function getNftCollectionIdConstant({
  ...
  if (constants.omnipool.nftCollectionId.v324.is(block)) {
    const resp = constants.omnipool.burnProtocolFee.v324.get(block);
    return {
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.omnipool.positions.v324.is(block)) {
        try {
          const resp = await storage.omnipool.positions.v324.getMany(
            block,
    ...
    fn: async () => {
      ...
      if (storage.omnipool.assets.v324.is(block)) {
        try {
          ...
          for await (const page of storage.omnipool.assets.v324.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
