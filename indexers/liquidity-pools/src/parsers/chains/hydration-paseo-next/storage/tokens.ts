...
async function getManyTokensTotalIssuance({
  ...
  if (storage.tokens.totalIssuance.v324.is(block)) {
    const resp = await storage.tokens.totalIssuance.v324.getMany(
      block,
      tokenIds.map((id) => +id)
    );
    if (!resp) return [];
...
async function getTokenBalancesMany({
  ...
  if (storage.tokens.accounts.v324.is(block) || block.specVersion >= 324) {
    return tryExecOrReturnFallback(async () => {
      ...
      for (const accountId of accountIds) {
        ...
        for await (const page of storage.tokens.accounts.v324.getPairsPaged(
          500,
          block,
          accountId
        ))
