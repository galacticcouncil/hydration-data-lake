...
async function getAllDepositsData({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.omnipoolWarehouseLm.deposit.v324.is(block)) {
        try {
          const pairsPaged: OmnipoolYieldFarmDepositDataWithId[] = [];

          for await (const page of storage.omnipoolWarehouseLm.deposit.v324.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
...
> {
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (
        storage.omnipoolWarehouseLm.deposit.v324.is(block) ||
        block.specVersion >= 324
      ) {
        try {
          const resp = await storage.omnipoolWarehouseLm.deposit.v324.getMany(
            block,
