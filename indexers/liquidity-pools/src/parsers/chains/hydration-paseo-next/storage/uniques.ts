...
async function getAssetsData({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.uniques.asset.v324.is(block)) {
        try {
          const resp = await storage.uniques.asset.v324.getMany(
            block,
    ...
    fn: async () => {
      ...
      if (storage.uniques.asset.v324.is(block)) {
        try {
          ...
          for await (const page of storage.uniques.asset.v324.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
