...
function getConstants({
  ...
  if (constants.dynamicFees.assetFeeParameters.v405.is(block)) {
    const resp = constants.dynamicFees.assetFeeParameters.v405.get(block);
    if (resp) assetFeeParameters = resp;
  }
  if (constants.dynamicFees.protocolFeeParameters.v405.is(block)) {
    const resp = constants.dynamicFees.protocolFeeParameters.v405.get(block);
...
async function getAssetFeesAll({
  ...
  if (storage.dynamicFees.assetFee.v405.is(block) || block.specVersion >= 405) {
    return tryExecOrReturnFallback(async () => {
      const pairsPaged = [];

      try {
        for await (const page of storage.dynamicFees.assetFee.v405.getPairsPaged(
          500,
          block
        ))
          pairsPaged.push(
