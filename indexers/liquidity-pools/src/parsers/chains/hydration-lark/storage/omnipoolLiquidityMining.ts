...
function getNftCollectionIdConstant({
  ...
  if (constants.omnipoolLiquidityMining.nftCollectionId.v405.is(block)) {
    const resp =
      constants.omnipoolLiquidityMining.nftCollectionId.v405.get(block);
    return {
...
async function getOmniPositionId({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      if (block.specVersion < 405) return null;
      if (
        storage.omnipoolLiquidityMining.omniPositionId.v405.is(block) ||
        block.specVersion >= 405
      ) {
        return tryExecOrReturnFallback(async () => {
          const resp =
            await storage.omnipoolLiquidityMining.omniPositionId.v405.get(
              block,
              BigInt(depositId)
            );
          if (!resp) return null;
