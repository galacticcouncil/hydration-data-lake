...
function getConstants({ block }: GetConstantsInput): LbpConstants | null {
  ...
  if (constants.lbp.repayFee.v405.is(block)) {
    const resp = constants.lbp.repayFee.v405.get(block);
    if (resp !== undefined) repayFee = resp;
  }
  if (constants.lbp.maxInRatio.v405.is(block)) {
    const resp = constants.lbp.maxInRatio.v405.get(block);
    if (resp !== undefined) maxInRatio = resp;
  }
  if (constants.lbp.maxOutRatio.v405.is(block)) {
    const resp = constants.lbp.maxOutRatio.v405.get(block);
    if (resp !== undefined) maxOutRatio = resp;
  }
  if (constants.lbp.minPoolLiquidity.v405.is(block)) {
    const resp = constants.lbp.minPoolLiquidity.v405.get(block);
    if (resp !== undefined) minPoolLiquidity = resp;
  }
  if (constants.lbp.minTradingLimit.v405.is(block)) {
    const resp = constants.lbp.minTradingLimit.v405.get(block);
...
async function getPoolData({
  ...
  if (storage.lbp.poolData.v405.is(block)) {
    const resp = await storage.lbp.poolData.v405.get(block, poolAddress);
...
async function getAllPoolsData({
  ...
  if (storage.lbp.poolData.v405.is(block)) {
    for await (const page of storage.lbp.poolData.v405.getPairsPaged(
      100,
      block
    ))
      pairsPaged.push(
...
async function getAllPoolIds({
  ...
  if (storage.lbp.poolData.v405.is(block)) {
    const ids = await storage.lbp.poolData.v405.getKeys(block);
