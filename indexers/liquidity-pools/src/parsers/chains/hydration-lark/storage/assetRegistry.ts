...
async function getAsset(
  ...
  if (storage.assetRegistry.assets.v405.is(block)) {
    const resp = await storage.assetRegistry.assets.v405.get(block, +assetId);
...
async function getAssetMany(
  ...
  if (storage.assetRegistry.assets.v405.is(block)) {
    const resp = await storage.assetRegistry.assets.v405.getMany(
      block,
      assetIds.map((id) => +id)
    );
    if (!resp) return [];
...
async function getAssetAll(
  ...
  if (storage.assetRegistry.assets.v405.is(block)) {
    ...
    for await (const page of storage.assetRegistry.assets.v405.getPairsPaged(
      100,
      block
    ))
      pairsPaged.push(
...
async function getErc20AssetContractAddress(
  ...
  if (storage.assetRegistry.assetLocations.v405.is(block)) {
    const resp = await storage.assetRegistry.assetLocations.v405.get(
      block,
      +assetId
    );
    // @ts-ignore
    ...
    fn: async () => {
      ...
      if (storage.assetRegistry.assetLocations.v405.is(block)) {
        return tryExecOrReturnFallback(async () => {
          for await (const page of storage.assetRegistry.assetLocations.v405.getPairsPaged(
            500,
            block
          )) {
