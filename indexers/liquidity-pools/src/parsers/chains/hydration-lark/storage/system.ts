...
async function getSystemAccount(
  ...
  if (storage.system.account.v405.is(block)) {
    const resp = await storage.system.account.v405.get(block, account);
    if (!resp) return null;
    ...
    fn: async () => {
      ...
      if (storage.system.account.v405.is(block)) {
        return tryExecOrReturnFallback(async () => {
          const resp = await storage.system.account.v405.getMany(
            block,
            accountIds
          );
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      if (storage.system.account.v405.is(block)) {
        ...
        for await (const page of storage.system.account.v405.getKeysPaged(
          500,
          block
        )) {
          resp.push(page);
