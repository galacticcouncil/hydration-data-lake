...
function getConstants({ block }: GetConstantsInput): XykConstants {
  ...
  if (constants.xyk.getExchangeFee.v405.is(block)) {
    const resp = constants.xyk.getExchangeFee.v405.get(block);
    if (resp) exchangeFee = resp;
  }
  if (constants.xyk.maxInRatio.v405.is(block)) {
    const resp = constants.xyk.maxInRatio.v405.get(block);
    if (resp !== undefined) maxInRatio = resp;
  }
  if (constants.xyk.maxOutRatio.v405.is(block)) {
    const resp = constants.xyk.maxOutRatio.v405.get(block);
    if (resp !== undefined) maxOutRatio = resp;
  }
  if (constants.xyk.minPoolLiquidity.v405.is(block)) {
    const resp = constants.xyk.minPoolLiquidity.v405.get(block);
    if (resp !== undefined) minPoolLiquidity = resp;
  }
  if (constants.xyk.minTradingLimit.v405.is(block)) {
    const resp = constants.xyk.minTradingLimit.v405.get(block);
    if (resp !== undefined) minTradingLimit = resp;
  }
  if (constants.xyk.nativeAssetId.v405.is(block)) {
    const resp = constants.xyk.nativeAssetId.v405.get(block);
    if (resp !== undefined) nativeAssetId = resp;
  }
  if (constants.xyk.oracleSource.v405.is(block)) {
    const resp = constants.xyk.oracleSource.v405.get(block);
...
async function getPoolAssets({
  ...
  if (storage.xyk.poolAssets.v405.is(block)) {
    return tryExecOrReturnFallback(async () => {
      const resp = await storage.xyk.poolAssets.v405.get(block, poolAddress);
...
async function getShareToken({
  ...
  if (storage.xyk.shareToken.v405.is(block)) {
    return tryExecOrReturnFallback(async () => {
      const resp = await storage.xyk.shareToken.v405.get(block, poolAddress);
...
async function getPoolShareTokenPairsMany({
  ...
  if (storage.xyk.shareToken.v405.is(block)) {
    return tryExecOrReturnFallback(async () => {
      ...
      try {
        for await (const page of storage.xyk.shareToken.v405.getPairsPaged(
          500,
          block
        )) {
          pairsPaged.push(
