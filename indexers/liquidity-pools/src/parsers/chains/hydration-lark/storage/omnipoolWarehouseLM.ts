...
> {
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (
        storage.omnipoolWarehouseLm.globalFarm.v405.is(block) ||
        block.specVersion >= 138
      ) {
        try {
          const resp =
            await storage.omnipoolWarehouseLm.globalFarm.v405.getMany(
              block,
...
async function getAllDepositsData({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.omnipoolWarehouseLm.deposit.v405.is(block)) {
        try {
          const pairsPaged: OmnipoolYieldFarmDepositDataWithId[] = [];

          for await (const page of storage.omnipoolWarehouseLm.deposit.v405.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
...
> {
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (
        storage.omnipoolWarehouseLm.deposit.v405.is(block) ||
        block.specVersion >= 405
      ) {
        try {
          const resp = await storage.omnipoolWarehouseLm.deposit.v405.getMany(
            block,
