...
function getConstants({ block }: GetConstantsInput): OmnipoolConstants {
  ...
  if (constants.omnipool.hdxAssetId.v405.is(block)) {
    const resp = constants.omnipool.hdxAssetId.v405.get(block);
    if (resp !== undefined) hdxAssetId = resp;
  }
  if (constants.omnipool.hubAssetId.v405.is(block)) {
    const resp = constants.omnipool.hubAssetId.v405.get(block);
    if (resp !== undefined) hubAssetId = resp;
  }
  if (constants.omnipool.maxInRatio.v405.is(block)) {
    const resp = constants.omnipool.maxInRatio.v405.get(block);
    if (resp !== undefined) maxInRatio = resp;
  }
  if (constants.omnipool.maxOutRatio.v405.is(block)) {
    const resp = constants.omnipool.maxOutRatio.v405.get(block);
    if (resp !== undefined) maxOutRatio = resp;
  }
  if (constants.omnipool.minimumPoolLiquidity.v405.is(block)) {
    const resp = constants.omnipool.minimumPoolLiquidity.v405.get(block);
    if (resp !== undefined) minPoolLiquidity = resp;
  }
  if (constants.omnipool.minimumTradingLimit.v405.is(block)) {
    const resp = constants.omnipool.minimumTradingLimit.v405.get(block);
    if (resp !== undefined) minTradingLimit = resp;
  }
  if (constants.omnipool.minWithdrawalFee.v405.is(block)) {
    const resp = constants.omnipool.minWithdrawalFee.v405.get(block);
...
function getNftCollectionIdConstant({
  ...
  if (constants.omnipool.nftCollectionId.v405.is(block)) {
    const resp = constants.omnipool.nftCollectionId.v405.get(block);
    return {
...
async function getOmnipoolAssetData({
  ...
  if (storage.omnipool.assets.v405.is(block)) {
    const resp = await storage.omnipool.assets.v405.get(block, assetId);
    return resp ?? null;
  }
...
async function getOmnipoolAllAssetIds({
  ...
  if (storage.omnipool.assets.v405.is(block)) {
    const resp = await storage.omnipool.assets.v405.getKeys(block);
...
async function getOmnipoolHubAssetTradability({
  ...
  if (storage.omnipool.assets.v405.is(block)) {
    const resp = await storage.omnipool.hubAssetTradability.v405.get(block);
...
> {
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.omnipool.positions.v405.is(block)) {
        try {
          const resp = await storage.omnipool.positions.v405.getMany(
            block,
    ...
    fn: async () => {
      ...
      if (storage.omnipool.positions.v405.is(block)) {
        try {
          ...
          for await (const page of storage.omnipool.positions.v405.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
    ...
    fn: async () => {
      ...
      if (storage.omnipool.assets.v405.is(block)) {
        try {
          ...
          for await (const page of storage.omnipool.assets.v405.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
