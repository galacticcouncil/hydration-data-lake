...
async function getAccountExtension({
  ...
  if (storage.evmAccounts.accountExtension.v405.is(block)) {
    const resp = await storage.evmAccounts.accountExtension.v405.get(
      block,
      evmAddress
    );
...
> {
  ...
  if (
    storage.evmAccounts.accountExtension.v405.is(block) ||
    ...
    return tryExecOrReturnFallback(async () => {
      try {
        ...
        for await (const page of storage.evmAccounts.accountExtension.v405.getPairsPaged(
          500,
          block
        )) {
          pairsPaged.push(
      ...
      try {
        const pairsPaged: EvmAccountsAccountExtensionWithEvmAddress[] = (
          await pMap(evmAddresses, async (h160Address) => {
            const extension =
              await storage.evmAccounts.accountExtension.v405.get(
                block,
                h160Address
              );
