...
async function getTokensAccountsAssetBalances(
  ...
  if (storage.tokens.accounts.v405.is(block)) {
    const resp = await storage.tokens.accounts.v405.get(
      block,
      account,
      assetId
    );
...
async function getTokenTotalIssuance({
  ...
  if (storage.tokens.totalIssuance.v405.is(block)) {
    const resp = await storage.tokens.totalIssuance.v405.get(block, tokenId);
    return resp ?? null;
  }
...
async function getManyTokensTotalIssuance({
  ...
  if (storage.tokens.totalIssuance.v405.is(block)) {
    const resp = await storage.tokens.totalIssuance.v405.getMany(
      block,
      tokenIds.map((id) => +id)
    );
    if (!resp) return [];
...
async function getTokenBalancesMany({
  ...
  if (storage.tokens.accounts.v405.is(block) || block.specVersion >= 405) {
    return tryExecOrReturnFallback(async () => {
      ...
      for (const accountId of accountIds) {
        ...
        for await (const page of storage.tokens.accounts.v405.getPairsPaged(
          500,
          block,
          accountId
        ))
