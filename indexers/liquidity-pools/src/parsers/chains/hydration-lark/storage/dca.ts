...
async function getDcaSchedule({
  ...
  if (storage.dca.schedules.v405.is(block)) {
    const resp = await storage.dca.schedules.v405.get(block, scheduleId);
...
async function getDcaSchedulesMany({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.dca.schedules.v405.is(block)) {
        const resp = await storage.dca.schedules.v405.getMany(
          block,
          scheduleIds
        );
