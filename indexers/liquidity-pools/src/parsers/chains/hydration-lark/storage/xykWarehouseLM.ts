...
async function getXykpoolLMDeposits({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (
        storage.xykWarehouseLm.deposit.v405.is(block) ||
        block.specVersion >= 405
      ) {
        try {
          const resp = await storage.xykWarehouseLm.deposit.v405.getMany(
            block,
...
async function getAllDepositsData({
  ...
  return measureStorageFetch({
    ...
    fn: async () => {
      ...
      if (storage.xykWarehouseLm.deposit.v405.is(block)) {
        try {
          const pairsPaged: XykpoolLMDepositDataWithId[] = [];

          for await (const page of storage.xykWarehouseLm.deposit.v405.getPairsPaged(
            500,
            block
          ))
            pairsPaged.push(
