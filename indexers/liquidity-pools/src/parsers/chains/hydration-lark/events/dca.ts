...
function parseScheduledParams(event: SqdEvent): DcaScheduledEventParams {
  if (events.dca.scheduled.v405.is(event)) {
    const { id, who } = events.dca.scheduled.v405.decode(event);
    return {
      id,
      who,
    };
...
function parseExecutionPlannedParams(
  ...
  if (events.dca.executionPlanned.v405.is(event)) {
    const {
      id,
      who,
      block: blockNumber,
    } = events.dca.executionPlanned.v405.decode(event);
    return {
      id,
      who,
      blockNumber,
...
function parseTradeExecutedParams(
  ...
  if (events.dca.tradeExecuted.v405.is(event)) {
    const { id, who, amountIn, amountOut } =
      events.dca.tradeExecuted.v405.decode(event);
    return {
      id,
      who,
      amountIn,
...
function parseTradeFailedParams(event: SqdEvent): DcaTradeFailedEventParams {
  if (events.dca.tradeFailed.v405.is(event)) {
    const { id, who, error } = events.dca.tradeFailed.v405.decode(event);
    return {
      id,
      who,
      // @ts-ignore
...
function parseTerminatedParams(event: SqdEvent): DcaTerminatedEventParams {
  if (events.dca.terminated.v405.is(event)) {
    const { id, who, error } = events.dca.terminated.v405.decode(event);
    return {
      id,
      who,
      // @ts-ignore
...
function parseCompletedParams(event: SqdEvent): DcaCompletedEventParams {
  if (events.dca.completed.v405.is(event)) {
    const { id, who } = events.dca.completed.v405.decode(event);
    return {
      id,
      who,
    };
...
function parseRandomnessGenerationFailedParams(
  ...
  if (events.dca.randomnessGenerationFailed.v405.is(event)) {
    const { block, error } =
      events.dca.randomnessGenerationFailed.v405.decode(event);
    return {
      block,
      // @ts-ignore
      error,
